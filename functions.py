...
def fi(a):
    try:
        ...
        driver.find_element(By.XPATH, value=a).text
    except:
        return False
    return True
...
def ffi10(a):
    c = driver.find_elements(by=By.XPATH, value=a)
    return len(c)


def ffi11(a):
    c = driver.find_element(by=By.XPATH, value=a)
    return c.get_attribute('x-page')
...
def get_data_typeA(i, link):
    ...
    n_book = ffi('//*[@id="tournamentTable"]/tbody/tr[{}]/td[6]')
    a = ffi2(target)
    ...
    if a:
        ...
        for j in range(1, n_book + 1):  # only first 10 bookmakers displayed
            Book = ffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[1]/div/a[2]'.format(j))  # first bookmaker name
...
def get_data_next_games_typeA(i, link):
    ...
    if a == True:
        ...
        # Now we collect all bookmaker
        for j in range(1,
                       30):  # only first 10 bookmakers displayed, CHANGE 01/05/2020 -> div[1] BECOMES div + WE STOP AT td[...] for odds
            Book = ffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[1]/div/a[2]'.format(j))  # first bookmaker name
...
def scrape_page_current_season_typeA(page, sport, country, tournament):
    link = f'https://www.oddsportal.com/{sport}/{country}/{tournament}/results/page/1/#/page/{page}'
    DATA = []
    for i in range(1, 100):
        content = get_data_typeA(i, link)
        if content != None:
...
def scrape_current_tournament_typeA(sport, tournament, country, SEASON, max_page=25):
    ...
    data_df.to_csv(f'./{tournament}_FULL/{tournament}_{SEASON}_FULL.csv', sep=';', encoding='utf-8', index=False)
    data_df[
        ['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].to_csv(
        f'./{tournament}/{tournament}_{SEASON}.csv', sep=';', encoding='utf-8', index=False)
...
def scrape_current_season_typeA(tournament, sport, country, SEASON, max_page=25):
    ...
    data_df.to_csv('./{}_FULL/{}_CurrentSeason_FULL.csv'.format(tournament, tournament), sep=';', encoding='utf-8',
                   index=False)
...
def scrape_league_typeA(Season, sport, country1, tournament1, nseason, current_season='yes', max_page=25):
    ...
    if current_season == 'yes':
        ...
        scrape_current_season_typeA(tournament=tournament1, sport=sport, country=country1, SEASON='CurrentSeason',
                                    max_page=max_page)
...
def scrape_next_games_typeA(tournament, sport, country, SEASON, nmax=30):
    ...
    data_df[
        ['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].to_csv(
        f'./{tournament}/NextGames_{tournament}_{SEASON}_08042020.csv', sep=';', encoding='utf-8', index=False)
...
def scrape_page_typeB(page, country, tournament, SEASON,
...
def scrape_current_tournament_typeB(Surface, bestof=3, tournament='wta-lyon', country='france',
                                    name_to_write='WTA Lyon', SEASON='2020', max_page=20):
    global driver
    REACH_END = 0
    ...
    FINAL_DATASET = pd.DataFrame({'Date': [y[1] for y in DATE],  # 'Time': [y[2] for y in DATE],
                                  ...
                                  'Score_home': [scores[y[0] + ' - ' + y[1]]['setP1'] for y in teams],
                                  # [y[0][len(y[0])-1:len(y[0])] for y in set_scores],
                                  'Score_away': [scores[y[0] + ' - ' + y[1]]['setP2'] for y in teams],
                                  # [y[1][0] for y in set_scores],
                                  'Set1score1': [scores[y[0] + ' - ' + y[1]]['jeux']['set1P1'] for y in teams],
                                  # [y[0] for y in set1games],
                                  'Set1score2': [scores[y[0] + ' - ' + y[1]]['jeux']['set1P2'] for y in teams],
                                  # [y[1] for y in set1games],
                                  'Set2score1': [scores[y[0] + ' - ' + y[1]]['jeux']['set2P1'] for y in teams],
                                  # [y[0] for y in set2games],
                                  'Set2score2': [scores[y[0] + ' - ' + y[1]]['jeux']['set2P2'] for y in teams],
    ...
    if not os.path.exists('./{}'.format(tournament)):
        os.makedirs('./{}'.format(tournament))
    FINAL_DATASET.to_csv('./{}/{}_{}.csv'.format(tournament, tournament, SEASON), sep=';', encoding='utf-8',
                         index=False)
    # print(os.listdir())
    # print(os.getcwd())
...
def scrape_next_games_typeB(Surface, bestof, tournament, country, name_to_write, SEASON='2020'):
    ...
    if not os.path.exists('./{}'.format(tournament)):
        os.makedirs('./{}'.format(tournament))
    FINAL_DATASET.to_csv('./{}/NextGames_{}_{}.csv'.format(tournament, tournament, SEASON), sep=';', encoding='utf-8',
                         index=False)
    # print(os.listdir())
    # print(os.getcwd())
    return (FINAL_DATASET)
...
def get_data_typeC(i, link):
    driver.get(link)
    target = '//*[@id="tournamentTable"]/tbody/tr[{}]/td[2]/a'.format(i)
    n_book = ffi('//*[@id="tournamentTable"]/tbody/tr[{}]/td[7]'.format(i))
    n_count = ffi9('deactivate')
    n_count += ffi9('nob-border')
    n_count += ffi9('table-dummyrow')
    page_1 = ffi10('//*[@id="pagination"]/a')
    page_1 = ffi11(f'//*[@id="pagination"]/a[{page_1}]')
    a = ffi2(target)
    print(n_book, a, n_count, page_1)
    if a:
        print('We wait 4 seconds')
        L = []
        time.sleep(4)
        # Now we collect all bookmaker
        for j in range(1, int(n_book) + 1):  # only first 10 bookmakers displayed
            Book = ffi(
                '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[1]/div/a[2]'.format(j))  # first bookmaker name
            Odd_1 = fffi('//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[2]/div'.format(j))  # first home odd
...
def scrape_page_typeC(page, sport, country, tournament, SEASON):
    link = 'https://www.oddsportal.com/{}/{}/{}-{}/results/page/1/#/page/{}'.format(sport, country, tournament, SEASON,
                                                                                    page)
    DATA = []
...
def scrape_current_tournament_typeC(sport, tournament, country, SEASON, max_page=25):
    ...
    data_df["Season"] = SEASON
    # Finally we save results
    if not os.path.exists(CWD + f'/{sport}/{tournament}_FULL'):
        os.makedirs(CWD + f'/{sport}/{tournament}_FULL')
    if not os.path.exists(CWD + f'/{sport}/{tournament}'):
        os.makedirs(CWD + f'/{sport}/{tournament}')

    data_df.to_csv(CWD + f'/{sport}/{tournament}_FULL/{tournament}_{SEASON}_FULL.csv', sep=';', encoding='utf-8',
                   index=False)
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddDraw', 'OddAway', 'Date', 'Score_home', 'Score_away',
             'Season']].to_csv(CWD + f'/{sport}/{tournament}/{tournament}_{SEASON}.csv', sep=';', encoding='utf-8',
                               index=False)
...
def scrape_current_season_typeC(tournament, sport, country, SEASON, max_page=25):
    ...
    data_df.to_csv(CWD + f'/{sport}/{tournament}_FULL/{tournament}_CurrentSeason_FULL.csv', sep=';', encoding='utf-8',
                   index=False)
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddDraw', 'OddAway', 'Date', 'Score_home', 'Score_away',
             'Season']]. \
        to_csv(CWD + f'/{sport}/{tournament}/{tournament}_CurrentSeason.csv', sep=';', encoding='utf-8', index=False)
    return (data_df)
...
def scrape_league_typeC(Season, sport, country1, tournament1, nseason, current_season='yes', max_page=25):
    ...
    if current_season == 'yes':
        ...
        if long_season:
            SEASON1 = '{}-{}'.format(Season, Season + 1)
        print('We start to collect current season')
        scrape_current_season_typeC(tournament=tournament1, sport=sport, country=country1, SEASON='CurrentSeason',
                                    max_page=max_page)
        print('We finished to collect current season !')

    # Finally we merge all files

    file1 = pd.read_csv(CWD + f'/{sport}/{tournament1}/' + os.listdir(CWD + f'/{sport}/{tournament1}/')[0], sep=';')
    print(os.listdir(CWD + f'/{sport}/{tournament1}/')[0])
    for filename in os.listdir(CWD + f'/{sport}/{tournament1}/')[1:]:
        file = pd.read_csv(CWD + f'/{sport}/{tournament1}/' + filename, sep=';')
        print(filename)
        file1 = file1.append(file)
...
def scrape_next_games_typeC(tournament, sport, country, SEASON, nmax=30):
    ...
    if not os.path.exists('./{}'.format(tournament)):
        os.makedirs('./{}'.format(tournament))
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddDraw', 'OddAway', 'Date', 'Score_home', 'Score_away',
             'Season']].to_csv('./{}/NextGames_{}_{}.csv'.format(tournament, tournament, SEASON), sep=';',
    ...
    if a == True:
        ...
        if a == True:
            ...
            for j in range(1, 15):  # only first 10 bookmakers displayed
                Book = ffi('//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[1]/div/a[2]'.format(
                    j))  # first bookmaker name
                Odd_1 = fffi(
                    '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[2]/div'.format(j))  # first home odd
                Odd_2 = fffi(
                    '//*[@id="odds-data-table"]/div[1]/table/tbody/tr[{}]/td[3]/div'.format(j))  # first away odd
                match = ffi('//*[@id="col-content"]/h1')  # match teams
                final_score = ffi('//*[@id="event-status"]')
                date = ffi('//*[@id="col-content"]/p[1]')  # Date and time
                print(match, Book, Odd_1, Odd_2, date, final_score, i, '/ 500 ')
...
def scrape_current_season_typeD(tournament, sport, country, SEASON, max_page=25):
    ...
    data_df[['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']]. \
        to_csv('./{}/{}_CurrentSeason_08042020.csv'.format(tournament, tournament), sep=';', encoding='utf-8',
               index=False)
    # 
...
def scrape_next_games_typeD(tournament, sport, country, SEASON, nmax=30):
    ...
    data_df[
        ['Home_id', 'Away_id', 'Bookmaker', 'OddHome', 'OddAway', 'Date', 'Score_home', 'Score_away', 'Season']].to_csv(
        './{}/NextGames_{}_{}_08042020.csv'.format(tournament, tournament, SEASON), sep=';', encoding='utf-8',
        index=False)
...
def scrape_league_typeD(Season, sport, country1, tournament1, nseason, current_season='yes', max_page=25):
    long_season = (
            len(Season) > 6)  # indicates whether Season is in format '2010-2011' or '2011' depends on the league)
    Season = int(Season[0:4])
    for i in range(nseason):
        SEASON1 = '{}'.format(Season)
        if long_season:
    ...
    if current_season == 'yes':
        ...
        if long_season:
            SEASON1 = '{}-{}'.format(Season, Season + 1)
        print('We start to collect current season')
        scrape_current_season_typeD(sport=sport, tournament=tournament1, country=country1, SEASON='CurrentSeason',
                                    max_page=max_page)
        print('We finished to collect current season !')
...
def scrape_oddsportal_current_season(sport='football', country='france', league='ligue-1', season='2019-2020',
    ...
    L = ['soccer', 'basketball', 'esports', 'darts', 'tennis', 'baseball', 'rugby-union', 'rugby-league',
    ...
    df.to_csv('./{}/CurrentSeason_{}_{}_{}.csv'.format(sport, country, league, season), sep=',', encoding='utf-8',
              index=False)
...
def scrape_oddsportal_next_games(sport='football', country='france', league='ligue-1', season='2019-2020', nmax=30):
    ...
    if sport in ['baseball', 'esports', 'basketball', 'darts', 'american-football', 'volleyball']:
        ...
        df = scrape_next_games_typeB(Surface=surface, bestof=bestof, tournament=league, country=country,
                                     name_to_write=league, SEASON='2020')
        df = create_clean_table_two_ways(df)
    elif sport in ['soccer', 'rugby-union', 'rugby-league', 'handball']:
    ...
    df.to_csv('./{}/NextGames_{}_{}_{}.csv'.format(sport, country, league, season), sep=',', encoding='utf-8',
              index=False)


def scrape_oddsportal_specific_season(sport='football', country='france', league='ligue-1', season='2019-2020',
                                      max_page=25):
    ''' sport : sport as mentioned on the oddsportal website
      country : country as mentioned on oddsportal website 
      league : league as mentioned on oddsportal website
    ...
    # df = create_clean_table_two_ways(df)
        ...
        df = scrape_current_tournament_typeC(sport=sport, tournament=league, country=country, SEASON=season,
                                             max_page=max_page)
        df = create_clean_table_three_ways(df)
    elif sport in ['hockey']:
        df = scrape_current_tournament_typeD(sport=sport, tournament=league, country=country, SEASON=season,
                                             max_page=max_page)
        df = create_clean_table_two_ways(df)
