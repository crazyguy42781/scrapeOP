...
def create_clean_table_two_ways(df):
    ...
    for i in range(1, df['Bookmaker'].size):
        if (df['Date'][i] != df['Date'][i - 1]) | (df['Home_id'][i] != df['Home_id'][i - 1]) | (
    ...
    df_final = pd.DataFrame(index=range(max(df['MatchId'])),
                            columns=range(7))  # Home_id, Away_id, Date, Score_home, Score_away, Season, MatchId
    df_final.columns = ['MatchId', 'Season', 'Home_id', 'Away_id', 'Date', 'Score_home', 'Score_away']
    c = 0
    for book in df['Bookmaker'].unique():
        print(book)
        df_final[f'{book}_H'] = 'Na'  # Home victory odds
        df_final[f'{book}_A'] = 'Na'  # Away victory odds
        for id in range(1, max(df['MatchId']) + 1):
            new_df = df[(df['Bookmaker'] == book) & (df['MatchId'] == id)]
            if new_df.shape[0] > 0:
                # print(id)
                df_final[f'{book}_H'].iloc[id - 1] = new_df['OddHome'].iloc[0]
                df_final[f'{book}_A'].iloc[id - 1] = new_df['OddAway'].iloc[0]
...
def create_clean_table_three_ways(df):
    ...
    for book in df['Bookmaker'].unique():
        print(book)
        df_final[f'{book}_H'] = 'Na'  # Home victory odds
        df_final[f'{book}_D'] = 'Na'  # Draw odds
        df_final[f'{book}_A'] = 'Na'  # Away victory odds
        for id in range(1, max(df['MatchId']) + 1):
            new_df = df[(df['Bookmaker'] == book) & (df['MatchId'] == id)]
            if new_df.shape[0] > 0:
                # print(id)
                df_final[f'{book}_H'].iloc[id - 1] = new_df['OddHome'].iloc[0]
                df_final[f'{book}_D'].iloc[id - 1] = new_df['OddDraw'].iloc[0]
                df_final[f'{book}_A'].iloc[id - 1] = new_df['OddAway'].iloc[0]
                df_final['MatchId'].iloc[id - 1] = new_df['MatchId'].iloc[0]
                df_final['Season'].iloc[id - 1] = new_df['Season'].iloc[0]
                df_final['Home_id'].iloc[id - 1] = new_df['Home_id'].iloc[0]
